  ...
  std::vector<std::unique_ptr<column>>& out_columns)
{
  ...
  std::for_each(
    ...
    [&](size_t i) {
      ...
      if (key_type.id() != type_id::STRING) {
        // Stack the fixed-width dictionary payloads into one buffer; only keys are copied, not row
        // indices. The copies are byte-wise on purpose: `chunk_dict_data[k]` points straight into
        // the (decompressed) Parquet page payload, which the format aligns to nothing, so a
        // `column_view` over it -- and the typed loads `cudf::detail::concatenate` issues through
        // `fused_concatenate_kernel` -- would fault with `cudaErrorMisalignedAddress`. A
        // `cudaMemcpyAsync` per chunk has no alignment requirement and launches no kernel.
        auto const key_width = cudf::size_of(key_type);
        rmm::device_buffer stacked_bytes{static_cast<std::size_t>(total_keys) * key_width,
                                         _stream,
                                         get_current_device_resource_ref()};
        for (size_t k = 0; k < chunk_indices.size(); ++k) {
          if (chunk_key_counts[k] == 0) { continue; }
          CUDF_CUDA_TRY(
            cudaMemcpyAsync(static_cast<uint8_t*>(stacked_bytes.data()) +
                              static_cast<std::size_t>(key_counts_prefix[k]) * key_width,
                            chunk_dict_data[k],
        ...
        stacked_keys_owner = std::make_unique<column>(
          key_type, total_keys, std::move(stacked_bytes), rmm::device_buffer{}, 0);
      } else if (contiguous) {
        stacked_keys_owner =
