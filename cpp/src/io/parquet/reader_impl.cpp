    ...
    _input_pass_read_limit{pass_read_limit}
{
  ...
  // Direct parquet-dict -> DICTIONARY32 transcode
  _options.output_dict_columns = options.is_enabled_output_dict_columns();
  // The direct fast path only supports single-pass, non-chunked reads.
  if (_options.output_dict_columns and
      (_output_chunk_read_limit != 0 or _input_pass_read_limit != 0)) {
    CUDF_LOG_WARN(
...
table_with_metadata reader_impl::read_chunk_internal(read_mode mode)
{
  ...
  // columns (INT8/INT16/INT32, sized to the dictionary). Assemble them into DICTIONARY32 columns
  // here by attaching per-chunk keys; concatenate remaps indices to the unified keys child.
