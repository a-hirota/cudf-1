  ...
  page_state_s* s, state_buf* const sb, int start, int end, int t)
{
  ...
  while (pos < end) {
    ...
    if (thread_pos < target_pos && dst_pos >= 0) {
      ...
      auto const entry_size = s->col.physical_type == Type::BYTE_ARRAY
                                ? sizeof(string_index_pair)
                                : static_cast<size_t>(s->dtype_len_in);
      auto const num_keys   = static_cast<uint32_t>(s->dict_size / entry_size);
...
/**
 * @brief Check whether the kernel mask decodes parquet dictionary indices directly to a signed
  ...
  if constexpr (has_strings_t || has_lists_t || is_dict_int32_t) {
    if (process_nulls) {
      uint32_t const dtype_len = [&]() -> uint32_t {
        if constexpr (is_dict_int32_t) { return s->col.dict_index_bytes; }
